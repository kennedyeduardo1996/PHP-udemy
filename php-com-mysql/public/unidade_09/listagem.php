...
if(isset($_GET["produto_pesquisa"])) {
    ...
    $sql = "SELECT nometransportadora, nome  FROM transportadoras t INNER JOIN estados e ON t.estadoID = e.estadoID WHERE  nometransportadora LIKE :pesquisa ;";
    $produtos = $con->prepare($sql);
    $produtos->bindParam(':pesquisa', $pesquisa);
} else {
    $sql = "SELECT nometransportadora, nome  FROM transportadoras t INNER JOIN estados e ON t.estadoID = e.estadoID;";
    $produtos = $con->prepare($sql);
}
        ...
        <div id="listagem_produtos">
            ...
            foreach($produtos as $result) { ?>
                <ul>
                    ...
                    <li><a href="alteracao.php?codigo=<?="test" ?>"></a>Alterar</li>
                </ul>
            <?php } ?>
        </div>
    </main>
